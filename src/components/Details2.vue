<template>
  <div class="Details">
    ...
    <div class="pic">
      <img :src="$store.state.goods[1].g_img">
    </div>
    <div class="title">
      <h1>{{ $store.state.goods[1].name }}</h1>
    </div>
    <div class="pri">
      <p>价格:￥{{ $store.state.goods[1].price }}</p>
    </div>
    <div class="addbton">
...
export default {
  ...
  methods: {
    addsth () {
      ...
      var clist = [{
        num: len,
        id: this.$store.state.goods[1].num,
        name: this.$store.state.goods[1].name,
        price: this.$store.state.goods[1].price,
        quantity: 0,
        img: this.$store.state.goods[1].g_img,
        checked: false
      }]
      var i
      ...
      if (len === 0) {
        ...
        for (i = 0; i < len; i++) {
          if (this.$store.state.goods[1].name === cardata[i].name) {
            break
          } else {
            flag = 1
          }
