<template>
  <div class="Details">
    ...
    <div class="pic">
      <img :src="$store.state.goods[2].g_img">
    </div>
    <div class="title">
      <h1>{{ $store.state.goods[2].name }}</h1>
    </div>
    <div class="pri">
      <p>价格:￥{{ $store.state.goods[2].price }}</p>
    </div>
    <div class="addbton">
      ...
      <el-tabs v-model="activeName"
        ...
        <el-tab-pane label="商品详情"
          ...
          <table>
            ...
            <tr>
              <td>厂家联系方式：4008322868</td>
              <td>储藏方法：阴凉干燥处存放，防潮、防异</td>
              <td>保质期：540 天</td>
            </tr>
            <tr>
              <td>品牌: 雷允上</td>
...
export default {
  ...
  methods: {
    addsth () {
      ...
      var clist = [{
        num: len,
        id: this.$store.state.goods[2].num,
        name: this.$store.state.goods[2].name,
        price: this.$store.state.goods[2].price,
        quantity: 0,
        img: this.$store.state.goods[2].g_img,
        checked: false
      }]
      var i
      ...
      if (len === 0) {
        ...
        for (i = 0; i < len; i++) {
          if (this.$store.state.goods[2].name === cardata[i].name) {
            break
          } else {
            flag = 1
          }
